...
<div>
  ...
  <div class="detail" v-else>
    ...
    <div class="detail-right">
      ...
      <mu-raised-button @click="$router.back()" primary>返回</mu-raised-button>
    </div>
  </div>
</div>
</template>
...
export default {
  ...
  methods: {
    ...
    getMovie(title) {
      ...
      this.$axios.get(`${searchUrl}${title}`)
        .then(res => {
          if (res.data) {
            return res.data
          }
        ...
        .then(res => {
          ...
          let movieId = res.subjects[0].id
          if (!!movieId) {
            this.$axios.get(`${movieUrl}/${movieId}`)
              .then(res => {
                if (!!res.data) {
                  this.movie = res.data
                  setTimeout(() => {
